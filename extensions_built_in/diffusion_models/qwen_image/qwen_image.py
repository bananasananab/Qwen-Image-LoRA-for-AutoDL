...
from safetensors.torch import load_file  # kept for potential fallback
...
class QwenImageModel(BaseModel):
    ...
    def load_model(self):
        ...
        if base_model_path.endswith(".safetensors"):
            ...
            if self.model_config.tokenizer_name_or_path == self.model_config.extras_name_or_path:
                self.model_config.tokenizer_name_or_path = base_model_path
            if self.model_config.text_encoder_name_or_path == self.model_config.extras_name_or_path:
                self.model_config.text_encoder_name_or_path = base_model_path
            if self.model_config.vae_name_or_path == self.model_config.extras_name_or_path:
                self.model_config.vae_name_or_path = base_model_path

        self.print_and_status_update("Loading transformer")

        if model_path.endswith(".safetensors"):
            # Load fp8 safetensors model
            # IMPORTANT: do NOT convert to bf16 — fp8 weights must stay fp8
            # or VRAM usage doubles (20GB -> 40GB) exceeding 32GB GPU

            # from_single_file has meta-tensor bug in diffusers 0.38+/accel 1.13+.
            # Skip it, go directly to manual fp8-preserving load.
            from huggingface_hub import hf_hub_download
            import json
            config_path = hf_hub_download(
                "Qwen/Qwen-Image", filename="transformer/config.json"
            )
            ...
            # Stream-load with ComfyUI key remapping
            # ComfyUI safetensors use "model.diffusion_model." prefix
            ...
            with safe_open(model_path, framework="pt", device="cpu") as f:
                for key in f.keys():
                    ...
                    if stripped in param_dict:
                        param_dict[stripped].data = f.get_tensor(key)
                        loaded += 1
            self.print_and_status_update(
                f"  Loaded {loaded} parameters (fp8 preserved)"
            )
            del param_dict
            import gc
            gc.collect()

            # DO NOT call transformer.to(model_dtype) -- it would upcast fp8 to bf16
            # and double VRAM usage
        ...
        tokenizer_kwargs = {"torch_dtype": dtype}
        if self.model_config.tokenizer_subfolder is not None:
            tokenizer_kwargs["subfolder"] = self.model_config.tokenizer_subfolder
        tokenizer = Qwen2Tokenizer.from_pretrained(
            self.model_config.tokenizer_name_or_path, **tokenizer_kwargs
        )

        text_encoder_kwargs = {"torch_dtype": dtype}
        if self.model_config.text_encoder_subfolder is not None:
            text_encoder_kwargs["subfolder"] = self.model_config.text_encoder_subfolder
        text_encoder = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            self.model_config.text_encoder_name_or_path, **text_encoder_kwargs
        )
        ...
        vae = AutoencoderKLQwenImage.from_pretrained(
            self.model_config.vae_name_or_path, **vae_kwargs
        )
